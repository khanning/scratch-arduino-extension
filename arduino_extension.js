...
(function(ext) {
  ...
  var blocks = {
    ...
    fr: [
      ['h', "Quand la carte est connectée", 'whenConnected'],
      [' ', 'Brancher la %m.hwOut au pin %n', 'connectHW', 'LED A', 3],
      [' ', 'Brancher le %m.hwIn au pin analogique %n', 'connectHW', 'potentiomètre', 0],
      ['-'],
      [' ', 'Mettre la %m.leds dans l\'état %m.output', 'digitalLED', 'LED A', 'allumé'],
      [' ', 'Mettre la luminosité de la %m.leds à %n%', 'setLED', 'LED A', 100],
      [' ', 'Ajouter %n% à la luminosité de la %m.leds', 'changeLED', '20', 'LED A'],
      ['-'],
      [' ', 'Orienter le %m.servos à %n degrés', 'rotateServo', 'servomoteur A', 180],
      [' ', 'Tourner le %m.servos de %n degrés', 'changeServo', 'servomoteur A', 20],
      ['-'],
      ['h', 'Quand le %m.buttons est %m.btnStates', 'whenButton', 'bouton A', 'appuyé'],
      ['b', '%m.buttons appuyé ?', 'isButtonPressed', 'bouton A'],
      ['-'],
      ['h', 'Quand le %m.hwIn est %m.ops %n%', 'whenInput', 'potentiomètre', '>', 50],
      ['r', 'Lire le %m.hwIn', 'readInput', 'potentiomètre'],
      ['-'],
      [' ', 'Mettre le pin %n dans l\'état %m.outputs', 'digitalWrite', 1, 'allumé'],
      [' ', 'Mettre le pin analogique %n à %n%', 'analogWrite', 3, 100],
      ['-'],
      ['h', 'Quand le pin %n est %m.outputs', 'whenDigitalRead', 1, 'allumé'],
      ['b', 'pin %n allumé?', 'digitalRead', 1],
      ['-'],
      ['h', 'Quand le pin analogique %n est %m.ops %n%', 'whenAnalogRead', 1, '>', 50],
      ['r', 'Lire le pin analogique %n', 'analogRead', 0],
      ['-'],
      ['r', 'Mapper %n entre %n et %n à entre %n et %n', 'mapValues', 50, 0, 100, -240, 240]
    ],
    it: [
  ...
  var menus = {
    ...
    fr: {
      ...
      hwOut: ['LED A', 'LED B', 'LED C', 'LED D', 'bouton A', 'bouton B', 'bouton C', 'bouton D', 'servomoteur A', 'servomoteur B', 'servomoteur C', 'servomoteur D'],
      leds: ['LED A', 'LED B', 'LED C', 'LED D'],
      outputs: ['allumé', 'éteint'],
      ops: ['>', '=', '<'],
      servos: ['servomoteur A', 'servomoteur B', 'servomoteur C', 'servomoteur D']
    },
    it: {
